...
public class MyStack<T> {
    ...
    public T pop(){
        if(isEmpty()) throw new EmptyCollectionException("Cannot pop from an empty stack");
        T top = (T) data[size - 1];
        data[size - 1] = null;
        size--;
        return top;
    ...
    @SuppressWarnings("unchecked")
    public T peek(){
        if(isEmpty()) throw new EmptyCollectionException("Cannot peek from an empty stack");
